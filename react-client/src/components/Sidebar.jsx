...
export const Sidebar = ({path}) => {
  ...
  return (
    <div className={`relative  ${isOpen ? 'w-[25vw] ' : 'w-[0px]'} min-h-screen bg-slate-900 `}>
      <button
        className={`py-4 px-1  text-black mt-2 bg-zinc-400 absolute ${isOpen ? 'translate-x-[19vw] rounded-br-md  rounded-tr-md rounded-tl-sm rounded-bl-sm' : 'translate-x-0 rounded-br-md  rounded-tr-md'} transition-transform transform  text-center z-[1]`}
        onClick={toggleSidebar}
      >
        {isOpen ? <MdArrowBackIos className='ml-1' /> : <MdArrowForwardIos className='ml-1' />}
      </button>
      <h2 className="text-white text-4xl  title-font font-semibold p-3">PowerCBI</h2>
